...
from economic_research_agent.sub_agents.tools.search_skill import web_search_skill
...
def test_web_search_skill_match():
    ...
    with patch('requests.post') as mock_post:
        ...
        mock_response.json.return_value = {
            'organic': [{'title': 'Found match for Austin', 'snippet': 'Raleigh electricity rates'}]
        }
...
def test_web_search_skill_generic():
    ...
    with patch('requests.post') as mock_post:
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'organic': [{'title': 'Detroit info', 'snippet': 'Unemployment stats'}]
        }
        mock_post.return_value = mock_response
