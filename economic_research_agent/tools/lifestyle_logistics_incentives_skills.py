...
class LogisticsRequest(BaseModel):
    city_names: List[str] = Field(..., description="List of city names to analyze logistics and shipping costs for.")
...
def get_logistics_efficiency(city_names: List[str]) -> str:
    ...
    for city in city_names:
        ...
        logistics_data = {
            "Austin": {"intermodal_access": "Tier 1", "shipping_cost_idx": "98 (Baseline 100)", "transit_reliability": "85%"},
            "Raleigh": {"intermodal_access": "Tier 2", "shipping_cost_idx": "104", "transit_reliability": "89%"},
            "San Francisco": {"intermodal_access": "World Class (Ports)", "shipping_cost_idx": "122", "transit_reliability": "78%"},
        }
        
        data = logistics_data.get(city_clean, {"intermodal_access": "N/A", "shipping_cost_idx": "N/A"})
        
        results.append({
            "City": city_clean,
            "Intermodal Hub Access": data["intermodal_access"],
            "Shipping Cost Index (Lower=Better)": data["shipping_cost_idx"],
            "Transit Reliability Rate": data.get("transit_reliability", "N/A"),
...
class LifestyleRequest(BaseModel):
    city_names: List[str] = Field(..., description="List of city names to fetch lifestyle benchmarks for.")
...
def get_cultural_amenity_score(city_names: List[str]) -> str:
    ...
    for city in city_names:
        ...
        lifestyle_data = {
            "Austin": {"walk_score": "42", "amenity_density": "Relatively High (Vibrant Hubs)", "safety_score": "Moderate"},
            "Raleigh": {"walk_score": "31", "amenity_density": "Moderate (Suburban Mix)", "safety_score": "Very High"},
            "San Francisco": {"walk_score": "89", "amenity_density": "World Class", "safety_score": "Relatively Low"},
        }
        
        data = lifestyle_data.get(city_clean, {"walk_score": "N/A", "amenity_density": "N/A"})
...
class IncentiveRequest(BaseModel):
    state_names: List[str] = Field(..., description="List of states to fetch tax incentive/subsidy benchmarks for.")
...
def get_regional_tax_incentives(state_names: List[str]) -> str:
    ...
    for state in state_names:
        ...
        incentive_data = {
            "Texas": {"top_program": "Chapter 313 (Semiconductor Abatement)", "subsidy_intensity": "Very High", "claws_back_policy": "Strict"},
            "North Carolina": {"top_program": "JDIG (Payroll Grant)", "subsidy_intensity": "High", "claws_back_policy": "Moderate"},
            "California": {"top_program": "California Competes (Tax Credit)", "subsidy_intensity": "Moderate", "claws_back_policy": "Very Strict"},
        }
        
        data = incentive_data.get(state, {"top_program": "N/A", "subsidy_intensity": "N/A"})
