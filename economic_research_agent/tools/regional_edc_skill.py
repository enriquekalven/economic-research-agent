...
class EDCRequest(BaseModel):
    state_names: List[str] = Field(..., description="List of states to fetch Economic Development Corporation (EDO) portal data for.")
...
def get_regional_edc_data(state_names: List[str]) -> str:
    ...
    for state in state_names:
        data = edo_data.get(state, {"portal": "N/A", "agency": f"State of {state} EDO", "highlights": "N/A"})
