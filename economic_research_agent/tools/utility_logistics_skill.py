...
class UtilityRequest(BaseModel):
    state_names: List[str] = Field(..., description="List of full state names to fetch utility/logistics data for.")
...
def get_industrial_infrastructure_stats(state_names: List[str]) -> str:
    ...
    for state in state_names:
        ...
        rates = {
            ...
            "North Carolina": {"elec_industrial_kwh": "$0.082", "renew_share": "15%"},
            "California": {"elec_industrial_kwh": "$0.145", "renew_share": "40%"},
            "Tennessee": {"elec_industrial_kwh": "$0.071", "renew_share": "12%"}
        }

        data = rates.get(state, {"elec_industrial_kwh": "N/A", "renew_share": "N/A"})
