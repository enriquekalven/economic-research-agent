...
def fetch_bea_regional_data(metro_names: List[str], report_type: str = "GDP") -> str:
    """
    ...
    """
    if not BEA_API_KEY:
        return json.dumps({"ERROR": "BEA_API_KEY not found in environment."}, indent=2)
            ...
            if fips:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        val_entries = data.get('BEAAPI', {}).get('Results', {}).get('Data', [])
                        if val_entries:
                            ...
                            results.append({"City": city, "Status": "No data items in BEA response."})
                    except Exception as e:
                        results.append({"City": city, "Status": f"Parsing Error: {str(e)}"})
                else:
                    results.append({"City": city, "Status": f"BEA API Failure ({response.status_code})"})
            else:
                results.append({"City": city, "Status": "MSA FIPS not found in Grounded Registry."})
