...
class TalentRequest(BaseModel):
    city_names: List[str] = Field(..., description="List of city names to fetch talent pipeline benchmarks for.")
    target_major: str = Field("Computer Science", description="Target academic major pipeline to analyze.")
...
def get_talent_pipeline_roi(
    ...
    for city in city_names:
        ...
        major_trends = {
            ...
            "San Francisco": {"grad_rate_3yr": "+5.2%", "patents": "4,820 (Yearly)"},
            "Seattle": {"grad_rate_3yr": "+9.8%", "patents": "3,410 (Yearly)"}
        }
        
        data = major_trends.get(city_clean, {"grad_rate_3yr": "N/A", "patents": "N/A"})
