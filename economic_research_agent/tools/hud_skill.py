...
def fetch_hud_fmr_data(county_fips: str) -> str:
    ...
    if not HUD_API_KEY:
        return json.dumps({"ERROR": "HUD_API_KEY environment variable is empty."}, indent=2)
    ...
    for year in ["2026", "2025", "2024"]:
        try:
            ...
            if response.status_code == 200:
                ...
                if rent:
                    return json.dumps({
                        "Geography": data_wrap.get('county_name', 'Unknown'),
                        "Rent_2BR": f"${float(rent):,.0f}",
                        "Year": year,
                        "Source": f"HUD User API (FMR/{year})"
                    }, indent=2)
            elif response.status_code == 401:
                return json.dumps({"ERROR": "HUD API Token Unauthorized (401). Check registration."}, indent=2)
        except Exception as e:
            continue
            
    return json.dumps({"ERROR": f"FMR lookup failed for FIPS {county_fips}. Verification required."}, indent=2)

def fetch_hud_income_limits(county_fips: str) -> str:
    """Fetches HUD Income Limits (AMI) with nested JSON schema matching."""
    if not HUD_API_KEY:
        return json.dumps({"ERROR": "HUD_API_KEY empty or invalid format."}, indent=2)
    ...
    for year in ["2025", "2024"]:
        try:
            ...
            if response.status_code == 200:
                ...
                income = very_low.get('il50_p1') or payload.get('il_data', {}).get('il50_4')
    ...
    return json.dumps({"ERROR": f"Income Limit lookup failed for FIPS {county_fips}."}, indent=2)
...
def analyze_housing_affordability(county_fips: str) -> str:
    ...
    if "ERROR" in fmr or "ERROR" in il:
        return json.dumps({"ERROR": f"HUD Analytics Pipeline Broken: {fmr.get('ERROR', '')} {il.get('ERROR', '')}"}, indent=2)
    ...
    try:
        ...
        verdict = "Severely Burdened" if burden_pct > 50 else "High Cost" if burden_pct > 30 else "Optimal"
