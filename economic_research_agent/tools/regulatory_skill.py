...
class RegulatoryRequest(BaseModel):
    state_names: List[str] = Field(..., description="List of states to check for regulatory activities.")
    industry_topic: str = Field("Semiconductor", description="Industry or topic to track (e.g. Energy, Zoning, Healthcare).")

def fetch_regulatory_notices(state_names: List[str], industry_topic: str = "Semiconductor") -> str:
    """
        ...
        for state in state_names:
            ...
            if response.status_code == 200:
                ...
                results.append({
                    ...
                    "Notices": state_results if state_results else "No recent filings found.",
                    "Source": "Federal Register (Live API)"
                })
            else:
                results.append({"State": state, "ERROR": f"Status {response.status_code}"})
