...
class PolicyRequest(BaseModel):
    state_names: List[str] = Field(..., description="List of states to fetch political/policy risk benchmarks for.")
...
def get_policy_risk_benchmarks(state_names: List[str]) -> str:
    ...
    for state in state_names:
        ...
        policy_data = {
            "Texas": {"lobby_climate": "Very Pro-Business", "regulatory_stability": "High", "upcoming_tax_sunsets": "Inventory Tax (2026)"},
            "North Carolina": {"lobby_climate": "Pro-Business", "regulatory_stability": "Very High", "upcoming_tax_sunsets": "None"},
            "California": {"lobby_climate": "Mixed/Regulated", "regulatory_stability": "Moderate", "upcoming_tax_sunsets": "R&D Credit (Expected)"},
        }
        
        data = policy_data.get(state, {"lobby_climate": "N/A", "regulatory_stability": "N/A"})
        
        results.append({
            "State": state,
            "Lobbying Climate Cluster": data["lobby_climate"],
            "Regulatory Stability Index": data["regulatory_stability"],
            "Upcoming Policy Sunsets": data.get("upcoming_tax_sunsets", "N/A"),
...
class ColRequest(BaseModel):
    city_names: List[str] = Field(..., description="List of city names to fetch cost-of-living adjustments (COLA) for.")
...
def get_purchasing_power_adjustment(city_names: List[str]) -> str:
    ...
    for city in city_names:
        ...
        coli_data = {
            "Austin": {"coli_index": "104.2", "housing_premium": "+15%", "real_income_multiplier": "0.96"},
            "Raleigh": {"coli_index": "92.4", "housing_premium": "-5%", "real_income_multiplier": "1.08"},
            "San Francisco": {"coli_index": "178.5", "housing_premium": "+110%", "real_income_multiplier": "0.56"},
        }
        
        data = coli_data.get(city_clean, {"coli_index": "N/A", "real_income_multiplier": "1.0"})
