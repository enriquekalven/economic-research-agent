...
def get_state_from_city(city: str) -> str:
    ...
    mapping = {
        ...
        "raleigh": "North Carolina", "durham": "North Carolina", "charlotte": "North Carolina",
...
def generate_metro_matrix_report(city_names: List[str]) -> str:
    ...
    labor_json = fetch_regional_macro_stats(city_names, series_type="unemployment")
    labor_data = json.loads(labor_json) if not labor_json.startswith("No FRED data") and not labor_json.startswith("ERROR") else []
    ...
    for city in city_names:
        sentiment_data.append({
            "City": city,
            "Business Climate News": analyze_market_sentiment(f"{city} business climate Forbes Forbes 500")
        })
